...
class GoalProgressService {
  ...
  static const String _userGoalsTable = 'user_goals';
  ...
  static Future<int> getGoalProgress(String goalId) async {
    try {
      // Try to get from Supabase first if user is online
      ...
      if (user != null) {
        try {
          ...
          if (response != null) {
            final progress = response['progress'] as int? ?? 0;
  ...
  static Future<void> updateGoalProgress(String goalId, int progress) async {
    try {
      ...
      // Try to sync with Supabase if user is online
      final user = UnifiedSupabaseAuthService.currentUser;
      if (user != null) {
        try {
          await _supabase
    ...
    try {
      ...
      if (user != null) {
        try {
          ...
          if (response != null) {
            final objectives = response['objectives'] as List<dynamic>? ?? [];
          ...
          debugPrint('Error fetching objectives from Supabase: $e');
        }
      }
